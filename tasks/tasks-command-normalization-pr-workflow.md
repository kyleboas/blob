...
- [x] 0.0 Create feature branch
  - [x] 0.1 Create and checkout a new branch for this feature (e.g., `git checkout -b feature/command-normalization-pr-workflow`)

- [x] 1.0 Define and implement composite command splitting (anti-stuck)
  - [x] 1.1 In `src/agent.ts`, add a “composite command detector” that identifies commands containing `&&` within the supported scope (start conservative).
  - [x] 1.2 Implement a safe splitter that produces an ordered list of steps (avoid splitting inside obvious quoted segments; if uncertain, do not split).
  - [x] 1.3 Add an execution harness that runs each step sequentially with per-step logging and stops on first failure.
  - [x] 1.4 Ensure the harness tracks a “current working directory” (`cd ...`) across steps without requiring `$()` substitution.
  - [x] 1.5 Ensure each step uses the existing timeout policy and returns partial progress + failing step in the final error summary.
  - [x] 1.6 Update log event types so operators can clearly see:
  ...
  - [x] 1.7 Add a regression test to ensure composite commands no longer appear “stuck” by validating multiple `command_executed` logs appear per run.

- [x] 2.0 Implement repo-agnostic PR workflow enforcement and rewrites (no push default branch, no gh, idempotent git)
  - [x] 2.1 Add rewrite rules to make git operations idempotent:
  ...
  - [x] 2.2 Block or rewrite attempts to use `gh`:
    - If `gh` appears in a command, rewrite to the supported PR creation path (see Task 3), or block with guidance.
  - [x] 2.3 Block attempts to write/modify `~/.netrc` (and similar credential files) at the safety layer.
  - [x] 2.4 Implement detection for “push default branch” patterns:
  ...
  - [x] 2.5 Ensure base branch is checked out deterministically before branching:
  ...
  - [x] 2.6 Ensure rewrites do not introduce disallowed patterns from `src/sandbox-client.ts` validation (`$(`, backticks, etc.).
  - [x] 2.7 Add tests covering:
...
- [x] 3.0 Implement reliable PR creation path (prefer repo helper; fallback to Worker-side GitHub API)
  - [x] 3.1 Detect whether repo-local helper exists in the working tree:
    - `github_tools.py` present and runnable with current python.
  - [x] 3.2 Implement a “push branch” path that prefers:
  ...
  - [x] 3.3 Implement PR creation that prefers:
    - `python github_tools.py create-pr ...`
  - [x] 3.4 Implement Worker-side GitHub API fallback (secure + most reliable):
  ...
  - [x] 3.5 Ensure PR creation fallback never logs tokens; sanitize error bodies before logging.
  - [x] 3.6 Implement repo context inference:
  ...
  - [x] 3.7 Add unit tests with mocked `fetch` for Worker-side PR creation fallback:
...
- [x] 4.0 Add token-protected `/diag/run` read-only production canary
  - [x] 4.1 Add `/diag/run` route in `src/index.ts`.
  - [x] 4.2 Implement authentication for `/diag/run`:
    - Require `Authorization: Bearer <token>` and compare to `DIAG_TOKEN` (new Worker secret).
  - [x] 4.3 Implement diag checks (read-only, no PRs, no pushes):
  ...
  - [x] 4.4 Return a structured JSON response:
    - `trace_id`, `ok`, per-check `{name, ok, ms, error?}`
  - [x] 4.5 Add tests for `/diag/run`:
    - unauthorized returns 401/403
    - authorized returns expected JSON structure
    - failure cases return `ok:false` with per-check error
  - [x] 4.6 Document required secrets and a sample curl command in `README.md`.

- [x] 5.0 Add/Update tests and documentation
  - [x] 5.1 Update `src/agent.test.ts` expectations impacted by:
  ...
  - [x] 5.2 Add new focused unit tests for:
