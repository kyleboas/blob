...
1. **As a developer**, I want to give the agent a task (e.g., "add a new tool for file search") and have it plan, implement, test, and commit the change — so I can extend the agent's capabilities without writing all the code myself.
2. **As a developer**, I want the agent to ask for my explicit approval before modifying its own core code — so I maintain control over what changes are applied.
...
6. **As a developer**, I want to deploy the agent to Fly.io and trigger sessions remotely — so it can run autonomously in a secure, isolated environment.
7. **As a developer**, I want the agent to fetch and cache documentation from allowlisted domains — so it can learn new API patterns and propose improvements based on up-to-date information.
8. **As a developer**, I want to configure which LLM model handles which task tier — so I can optimize cost vs. quality tradeoffs.
...
17. The system must prompt the operator via CLI stdin before applying any change to agent core files (`agent.py`, `sandbox.py`, `tools.py`, system prompt).
18. The approval interface must be designed behind an abstraction so it can be replaced with webhook, Slack, or PR-based approval in the future.
...
31. The system must read tasks from a `tasks.json` file.
32. The system must execute the improvement cycle: pick next task → read docs → plan → write code → test → commit + update `AGENT.md` (on pass) or rollback + log (on fail).

### 4.9 Deployment

33. The system must be deployable to Fly.io via a `Dockerfile` and `fly.toml`.
34. The system must use a persistent Fly.io volume for the git repo, knowledge files, and cached docs.
35. The system must store API keys as environment variables (never in code or committed files).
36. The system must enable `auto_stop_machines` for scale-to-zero when idle.
37. The system must provide a trigger mechanism (CLI command or webhook) to start agent sessions.
...
- **Web UI or dashboard** — interaction is CLI/webhook only.
...
- **Pluggable approval interface**: design the approval gate as a callable (e.g., `ApprovalGate` protocol/ABC) so CLI stdin can be swapped for HTTP, Slack, or PR-based gates later.

## 7. Technical Considerations

- **Language**: Python (first-class Anthropic SDK, trivial self-modification via `open(__file__)` / `importlib.reload()`).
- **Dependencies**: `anthropic` SDK, `docker` Python SDK (dev only), `flyctl` CLI (deployment).
...
3. **Session trigger mechanism**: should v0.1 use a simple CLI entrypoint (`python agent.py "task description"`), a webhook, or both?
