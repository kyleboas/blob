...
describe("worker entry point", () => {
  ...
  it("renders the live logs page for a thread", async () => {
    ...
    const response = await worker.fetch(new Request("https://example.com/live-logs?thread_ts=1711111111.3333"), env, ctx);
  ...
  it("proxies live log data through durable objects", async () => {
    ...
    const response = await worker.fetch(new Request("https://example.com/live-logs/data?thread_ts=1711111111.4444"), env, ctx);
